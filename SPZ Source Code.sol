...
\
//Sportz Coin will provide 4% Rewards paid in BUSD to your wallets on every transaction made. Other unique functions to benefit the future growth of $SPZ: 1% buyback burn, 4% marketing, 1% sustainable economic development projects, 5% liquidity pool.\'a0 \
\
...
\
    ...
    uint256 public swapTokensAtAmount = 777_777_770 * (10**9);\
...
\
    ...
    \
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee);\
\
    address public _marketingWalletAddress = 0xEF28EcB7f1372C802369B0FBAD1c1f15177e8dD0;\
    address public _developerWalletAddress = 0x2D1232a4E1E2F546Be152E96CbA41eEe5E92294f;\
\
...
\
    constructor() public BEP20("Sportz Coin", "SPZ") \{\
\
...
\
    ...
    \}\
\
    function setDeveloperFee(uint256 value) external onlyOwner\{\
        developerFee = value;\
     totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee);\
    \}   \
\
...
\
            checkForBuyBack();\
\
            uint256 marketingTokens = contractTokenBalance.mul(marketingFee+developerFee).div(totalFees);\
...
\
        uint256 marketingShare = newBalance.div(marketingFee+developerFee).mul(marketingFee);\
        IBEP20(BUSD).transfer(_marketingWalletAddress, marketingShare);\
        IBEP20(BUSD).transfer(_developerWalletAddress, newBalance.sub(marketingShare));\
    \}\
\
...
\
    ...
    uint256 public maxLimit =  _totalSupply.div(100).mul(1); //1%\
\
...
\
    constructor() public DividendPayingToken("SportzCoin_Dividen_Tracker", "SportzCoin_Dividend_Tracker") \{\
        claimWait = 3600;\
